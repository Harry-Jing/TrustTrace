...
<template>
  <div class="fixed right-4 bottom-4 z-50 flex flex-col items-end gap-1.5">
    ...
    <Transition name="dev-menu">
      <div
        v-if="open"
        class="mb-1 flex flex-col gap-1 rounded-lg bg-foreground/90 p-1.5 shadow-lg backdrop-blur-sm"
      >
        <button
          v-for="page in pages"
          :key="page.name"
          type="button"
          class="rounded-md border-none px-3 py-1.5 text-left font-mono text-label tracking-wide transition-colors duration-150"
          :class="
            currentPage === page.name
              ? 'bg-accent font-semibold text-card'
              : 'bg-transparent text-background/70 hover:bg-background/10 hover:text-background'
          "
    ...
    <button
      type="button"
      class="flex size-9 items-center justify-center rounded-full border-none bg-foreground/80 text-background shadow-lg backdrop-blur-sm transition-all duration-200 hover:scale-110 hover:bg-foreground"
