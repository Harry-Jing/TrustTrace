...
import type { CreateCheckResponseDto, ProgressEventDto } from "../types";
...
function createTestContext(pipelineDelayMs = 1): TestContext {
...
describe("TrustTrace server API", () => {
  it("creates a running check and returns the persisted record", async () => {
    const { services } = createTestContext(25);
  ...
  it("streams progress events and supports afterSeq replay", async () => {
    const { services } = createTestContext(1);
