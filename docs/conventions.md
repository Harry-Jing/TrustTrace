...
- Evidence discovery currently uses OpenAI Responses API web search as the P1.0 primary discovery path. OpenAI may parse the main claim, suggest query plans/candidate URLs, assess verified excerpts, and write constrained result copy; backend URL safety, input/source extraction, authority-aware ranking, same-domain dedupe, snippet-only downgrading, persistence, and deterministic synthesis remain the evidence gate.
- P1.5 discovery should become a user-selectable strategy with an allowlist of `search_api` and `llm_web`; do not add `auto` or `parallel` modes. `search_api` uses a dedicated web search provider such as Tavily, and `llm_web` uses LLM web search. Strategy selection only controls candidate URL discovery and must never bypass URL safety, extraction, snippet-only downgrading, persistence, source assessment, or deterministic synthesis. Do not silently switch strategies when the selected provider path is unavailable; return a provider configuration or discovery error instead.
- User-facing discovery strategy values belong in shared request/response contracts when exposed through the API. Do not accept arbitrary provider names from the frontend; map allowlisted strategy values to server-owned provider configuration.
